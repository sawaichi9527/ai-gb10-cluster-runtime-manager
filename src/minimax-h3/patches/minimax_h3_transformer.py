...
# Modified 2026 by Joey Rodriguez for NVIDIA GB10 / SM121 compatibility.
# Upstream source: vllm-project/vllm-omni, MiniMax H3 transformer module.
# Changes are documented in ../docs/PATCH.md and marked in the Git history.
"""MiniMax H3 packed-token audio/video DiT for vLLM-Omni.

The architecture and numerical contract are adapted from the Apache-2.0
SGLang diffusion reference supplied with the checkpoint.  vLLM tensor
parallel linears and the unified attention layer provide TP and Ulysses/Ring
sequence parallel execution without changing the checkpoint layout.
"""
...
def _bind_native_cuda_fp8_activation_quantizers(root: nn.Module) -> int:
    """Bypass vLLM's independently compiled QuantFP8 native wrapper.

    The MiniMax H3 image's Triton build cannot lower ``float8e4nv`` scalar
    constants for SM121.  Its CUDA ``scaled_fp8_quant`` custom op is supported,
    so bind every online-FP8 linear's activation quantizer to that path.
    """
    bound = 0
    for module in root.modules():
        quant_method = getattr(module, "quant_method", None)
        fp8_linear = getattr(quant_method, "fp8_linear", None)
        quantizer = getattr(fp8_linear, "quant_fp8", None)
        forward_cuda = getattr(quantizer, "forward_cuda", None)
        if quantizer is None or not callable(forward_cuda):
            continue
        quantizer._forward_method = forward_cuda
        bound += 1
    return bound
...
def _prepare_h3_checkpoint_weight(
    name: str,
    weight: torch.Tensor,
    *,
    arch: MiniMaxH3DiTArchConfig,
) -> torch.Tensor:
    """Normalize fused H3 checkpoint layouts before vLLM loads them.

    Keep layout conversion outside ``Parameter.weight_loader``. Online FP8
    temporarily wraps those loaders and later replays their full signatures;
    ...
    if name.endswith(".mlp.fc1.weight") and weight.shape[0] % 2:
        raise ValueError(
            "MiniMax H3 fc1 checkpoint rows must split evenly into "
            f"gate/up matrices, got {tuple(weight.shape)}"
        )
    return weight
...
class MiniMaxH3AdalnProj(nn.Module):
    ...
    def forward(self, t_emb: torch.Tensor) -> tuple[torch.Tensor, ...]:
        ...
        # Online FP8 replaces ``weight`` with an FP8 parameter after loading;
        # activations must remain BF16 and be quantized by the linear kernel.
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def __init__(
        ...
        if quant_config is not None:
            bound_quantizers = _bind_native_cuda_fp8_activation_quantizers(self)
            if bound_quantizers == 0:
                raise RuntimeError(
                    "MiniMax H3 FP8 requested, but no CUDA activation quantizers were found."
                )
            logger.info(
                "MiniMax H3 bound %d FP8 activation quantizers to native CUDA.",
                bound_quantizers,
            )
    ...
    def load_weights(
        ...
        for name, loaded_weight in weights:
            ...
            if param is None:
                ...
                continue
            loaded_weight = _prepare_h3_checkpoint_weight(
                name,
                loaded_weight,
                arch=self.arch,
            )
